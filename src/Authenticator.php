...
class Authenticator 
{
    ...
    public function __construct($configuration, $authenticationFailedHandler = null) {
        
        // Failed handler dependancy injection.
        $this->authenticationFailedHandler = $authenticationFailedHandler;
        
        if (empty($authenticationFailedHandler)) {
            $this->authenticationFailedHandler = new AuthenticationFailedHandler();
        }
    ...
    public function Authenticate() {
        ...
        $this->authenticationFailedHandler->handle($this->GetQuestion(), $this->formPath);
        return false;
    }
    ...
    private function OriginatedFromWhitelistedIP() {
        foreach ($this->whitelistedIPs as $ipAddress) {
